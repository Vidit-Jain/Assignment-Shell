...
int noOfTabs(String fileLine)
{
	...
	for (int i = 0; i < fileLine.length; i++) {
		if ((int)fileLine.str[i] != ASCII_TAB)
			break;
...
int *countIndents(String assignmentName, String fileName, int lines)
{
	...
	while ((fileRead = getline(&(fileLine->str), &lineLength, fp)) != -1) {
		...
		String *folderName =
			makeString(&(fileLine->str[indentCount[currLine]]));

		// Checks if the first folder of the setup corresponds to the assignment
		// we're in
...
void createFileStructure(String fileName, int *indentCount, int lines)
{
	...
	while ((fileRead = getline(&(fileLine->str), &lineLength, fp)) != -1) {
		...
		folderName =
			attachString(folderName->str, &(curr->str[indentCount[curr_line]]));
 ...
 * 1 - Improper indentation (Eg Current Line is indented 2 times more than the
 * prev line) 2 - Multiple Assignments (More than one line has zero indents,
 * which isn't allowed) 3 - Both 1 and 2 have occurred 4 - If any of the file
 * names are invalid 5 - If the assignment name specified in the first line
...
void setup(String assignmentName)
{
	...
	if (!setupExists(assignmentName))
		return; // Don't try to setup if the file doesn't exist
