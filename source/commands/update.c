...
void updateAssignment(String *serverPath,String *assignment)
{
    ...
    folder = attachString(folder->str, assignment->str);//path of the assignment on the server, if exists
    ...
    else // the assignment exists in the local machine
    {
        ...
        for(int i = 0; i < numberOfLines ; i++)
        {
            char pdfName[100];
            fscanf(fp, "%s" , pdfName);
            String *pdfString = makeString(&pdfName[2]); // Deletes all the files before copying over
            char *token;
        ...
        deleteFile(*makeString("pdfNames.txt")); // Remove temporary file created
